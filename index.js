...
function addGamesToPage(games) {
    ...
    for(const key in games) {
        ...
        element.innerHTML = `
            <img src="${games[key]["img"]}" class="game-img"/>
            <h1>${games[key]["name"]}</h1>
            <p>${games[key]["description"]}<p>
            <p>goal: $${games[key]["goal"].toLocaleString('en-US')}</p>
            <p>pledged: $${games[key]["pledged"].toLocaleString('en-US')}</p>
        `;
...
const totalRaised = GAMES_JSON.reduce( (amount, game) => {
...
has been raised for ${GAMES_JSON.reduce((count, game) => {return game.pledged > 0 ? count + 1 : count;}, 0)} games. 
Currently, ${ unfundedGames > 1 ? unfundedGames + " games remain" : unfundedGames + " game remains"} unfunded. We need your help to fund these amazing games!`;
...
secondElement.innerHTML = runnerUp.name;
...
function search() {
    ...
    deleteChildElements(gamesContainer);
    match.length > 0 ? addGamesToPage(match) : addGamesToPage(GAMES_JSON);
}
